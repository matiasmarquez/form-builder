...
const resolvedAriaLabel = computed(() => {
  ...
  return typeof fromAttr === 'string' && fromAttr.trim() !== '' ? fromAttr : undefined;
});
...
const variantClass = computed(() => {
  switch (props.variant) {
    ...
    case 'secondary':
      return 'border border-border-strong bg-surface-elevated text-fg hover:bg-surface';
    case 'ghost':
...
<template>
  <button
    ...
    class="inline-flex items-center justify-center gap-2 rounded-md font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-40"
